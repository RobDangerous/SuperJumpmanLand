...
{
{
	HX_SOURCE_POS("src/com/ktxsoftware/kje/backend/cpp/Painter.hx",8)
	this->tx = (int)0;
	HX_SOURCE_POS("src/com/ktxsoftware/kje/backend/cpp/Painter.hx",9)
	this->ty = (int)0;
}
;
	return null();
...
Void Painter_obj::translate( double x,double y){
{
		...
		HX_SOURCE_POS("src/com/ktxsoftware/kje/backend/cpp/Painter.hx",21)
		this->tx = x;
		HX_SOURCE_POS("src/com/ktxsoftware/kje/backend/cpp/Painter.hx",22)
		this->ty = y;
	}
return null();
}
...
Void Painter_obj::drawImage2( ::com::ktxsoftware::kje::Image image,double sx,double sy,double sw,double sh,double dx,double dy,double dw,double dh){
{
		...
		::com::ktxsoftware::kje::backend::cpp::Image_obj* img = dynamic_cast<::com::ktxsoftware::kje::backend::cpp::Image_obj*>(image->__GetRealObject());
		//haxePainter->drawRect(tx + dx, ty + dy, dw, dh, Kt::Color(0xff, 0, 0));
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("com.ktxsoftware.kje.backend.cpp.Painter"), hx::TCanCast< Painter_obj> ,sStaticFields,sMemberFields,
